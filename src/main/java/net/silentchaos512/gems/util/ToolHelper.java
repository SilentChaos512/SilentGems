...
public class ToolHelper {
    ...
    /**
     * A fake material for tools. Tools need a tool material, even if it's not used. Unfortunately,
     * some mods still reference the tool material (such as Progressive Automation, which is why I
     * chose harvest level 1).
     */
    ...
    // NBT example keys
    /**
     * Indicates a tool being displayed in JEI/creative/etc. These need a UUID, but a new UUID
     * should be generated if a copy of the tool makes its way into the player's inventory.
     */
    public static final String NBT_EXAMPLE_TOOL = "ExampleToolItem";
    /**
     * Indicates a tool being displayed in an example recipe. This adds some more descriptive lines
     * to the tooltip, instead of the generic "You broke it!" line.
     */
    ...
    /**
     * Recalculate all stats and properties, including the rendering cache for the given tool. In
     * general, this should be called any time changes are made to a tool (aside from incrementing
     * statistics, or something like that). For example, this is called during construction,
     * decoration, and for all tools in the players inventory during login.
     *
     * @param toolOrArmor
     */
    ...
    /**
     * Gets the tool's or armor's UUID. If it currently does not have a UUID, one will be created if
     * appropriate.
     *
     * @param tool The tool or armor stack. The stack's item must implement either ITool or IArmor.
    ...
    public static boolean hasUUID(ItemStack tool) {
        return !tool.isEmpty() && tool.hasTagCompound() && tool.getTagCompound().hasUniqueId(NBT_UUID);
    }
    ...
    public static boolean isExampleItem(ItemStack tool) {
        return !tool.isEmpty() && tool.hasTagCompound() && tool.getTagCompound().getBoolean(NBT_EXAMPLE_TOOL);
    }

    /**
     * Determine if tool is the output of an "example recipe" (the output of the recipes that show
     * in JEI).
     *
    ...
    public static boolean isExampleRecipeOutput(ItemStack tool) {
        return !tool.isEmpty() && tool.hasTagCompound() && tool.getTagCompound().getBoolean(NBT_EXAMPLE_TOOL_TIER);
    }
    ...
    /**
     * Check if the tool has no construction parts. Only checks the head, but no valid tools will
     * have no head parts.
     *
     ...
     *
     * @return False in all cases, because this method is only called when Item.onBlockStartBreak
     * returns false.
     */
    ...
    public static void toggleSpecialAbility(ItemStack tool) {
        boolean currentValue = getTagBoolean(tool, NBT_ROOT_PROPERTIES, NBT_SETTINGS_SPECIAL);
        setTagBoolean(tool, NBT_ROOT_PROPERTIES, NBT_SETTINGS_SPECIAL, !currentValue);
    }
