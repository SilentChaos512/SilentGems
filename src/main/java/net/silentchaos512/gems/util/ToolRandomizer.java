...
//    }
    ...
    public ItemStack randomize(ItemStack tool, float superChance) {
        ...
        ItemStack rod;
        if (superTier) {
            ItemStack[] choices = new ItemStack[]{CraftingItems.ORNATE_GOLD_ROD.getStack(), CraftingItems.ORNATE_SILVER_ROD.getStack(), CraftingItems.IRON_ROD.getStack()};
            rod = choices[SilentGems.random.nextInt(choices.length)];
        } else {
            ItemStack[] choices = new ItemStack[]{new ItemStack(Items.STICK), new ItemStack(Items.BONE), CraftingItems.IRON_ROD.getStack()};
            rod = choices[SilentGems.random.nextInt(choices.length)];
        }
