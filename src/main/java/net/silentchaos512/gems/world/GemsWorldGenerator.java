...
public class GemsWorldGenerator extends WorldGeneratorSL {
    ...
    /**
     * Selects an IBlockState for the config. For gems, it will normally choose one randomly each
     * call. For essence ores, it selects the appropriate ore.
     *
     * @param config
     * @param random
     * @return
    ...
    private void generateFlowers(World world, Random random, int posX, int posZ) {
        if (GemsConfig.GLOW_ROSE_DIMENSION_BLACKLIST.contains(world.provider.getDimension()))
            return;
    ...
    private void generateChaosNodes(World world, Random random, int posX, int posZ) {
        if (GemsConfig.CHAOS_NODE_DIMENSION_BLACKLIST.contains(world.provider.getDimension()))
            return;
