    ...
    public void update() {
        if (!world.isRemote) {
            ...
            if (world.getTotalWorldTime() % SEND_CHAOS_DELAY == 0) {
                List<IChaosAccepter> accepters = ChaosUtil.getNearbyAccepters(world, pos, SEARCH_RADIUS_BLOCK, SEARCH_RADIUS_BLOCK);
                List<EntityPlayer> players = world.getEntities(EntityPlayer.class, e -> e.getDistanceSq(pos) < SEARCH_RADIUS_PLAYER_SQUARED);
