...
public class EntityChaosProjectileHoming extends EntityChaosProjectile {
    ...
    public EntityChaosProjectileHoming(EntityLivingBase shooter, ItemStack castingStack, float damage, boolean flyUpBeforeHoming, double homingTightness, double homingSpeed) {
        ...
        if (flyUpBeforeHoming) {
            ...
            vec = new Vec3d(vec.x, 2, vec.z).normalize().scale(0.25 + 1.5 * SilentGems.instance.random.nextDouble());
        }
    ...
    public void findHomingTarget() {
        ...
        Predicate<EntityLivingBase> predicate = input -> shooter != null && shooter != input && !input.isOnSameTeam(shooter);
        ...
        for (int i = 0; i < world.loadedEntityList.size(); ++i) {
            ...
            if (entity instanceof EntityLivingBase) {
                ...
                if (predicate.test(entityLiving)) {
                    ...
                    int distance = (int) entityLiving.getDistanceSq(projectile) + SilentGems.random.nextInt(512);
