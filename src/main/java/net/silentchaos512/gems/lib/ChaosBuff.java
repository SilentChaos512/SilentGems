...
public class ChaosBuff {
    ...
    /**
     * The duration to apply. By default, this is done per tick, but there are exceptions. Note that
     * smaller durations are better. I use 20 ticks for most, but night vision requires more than
     * 400 ticks to prevent the flashing effect. Regeneration also has a special exception.
     *
    ...
    static {
        ...
        if (ModList.get().isLoaded("toughasnails")) {
            Potion coldResist = Potion.REGISTRY.get(new ResourceLocation("toughasnails:cold_resistance"));
            Potion heatResist = Potion.REGISTRY.get(new ResourceLocation("toughasnails:heat_resistance"));
            Potion thirst = Potion.REGISTRY.get(new ResourceLocation("toughasnails:thirst"));
    ...
    static final Potion hyperthermia = Potion.REGISTRY.get(new ResourceLocation("toughasnails:hyperthermia"));
    static final Potion hypothermia = Potion.REGISTRY.get(new ResourceLocation("toughasnails:hypothermia"));
    ...
    public void applyToPlayer(EntityPlayer player, int level, ItemStack stack) {
        ...
        if (this == FLIGHT) {
            ...
            if (data != null)
                data.flightTime = 100;
            if (!player.world.isRemote && player.world.getGameTime() % 20 == 0) {
    ...
    public void removeFromPlayer(EntityPlayer player, int level, ItemStack stack) {
        ...
        if (this == FLIGHT && !player.abilities.isCreativeMode) {
    ...
    /**
     * @return The number of slots used. Each level beyond 1 adds a slot (or subtracts it if
     * slotsUsed is negative)
     */
     ...
     *
     * @param level  The buff level. Should be greater than zero.
     * @param player The player (can be null). Cost can depend on the player's state. If null, this
     *               should return the maximum possible cost.
     * @return The amount of Chaos drained per tick by this buff. Each level beyond 1 adds 20% to
     * the cost.
     */
    public int getChaosCost(int level, @Nullable EntityPlayer player) {
        ...
        if (this == FLIGHT && player != null) {
            boolean notFlying = !player.abilities.isFlying || player.abilities.isCreativeMode;
    ...
    /**
     * @return The duration to apply for this effect. In most cases, this is just applyDuration, but
     * some effects have special requirements.
     */
