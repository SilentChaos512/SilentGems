...
//        .setFavorWeightMulti(0.25f);
    ...
    /**
     * If not empty, souls with matching elements are more likely to learn this skill. Souls without
     * one of the favored elements are less likely to learn the skill.
     */
    ...
    /**
     * Weight is multiplied by this when selecting a level-up skill. Set less than 1 to make the
     * skill rarer.
     */
...
//            SoundCategory.BLOCKS, 0.5f, 2.0f + (float) (0.2 * random.nextGaussian()));
    ...
    public String getLocalizedName(ItemStack tool, int level) {
        String strName = SilentGems.i18n.translate("skill", id + ".name");
        if (maxLevel == 1 || level == 0) {
            return strName;
        }
        String strNum = level > 10 ? Integer.toString(level) : SilentGems.i18n.translate("enchantment.level." + level);
